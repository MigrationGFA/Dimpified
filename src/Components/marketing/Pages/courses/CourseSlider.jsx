...
const CourseSlider = ({ recommended, popular, trending, category }) => {
  ...
  // useEffect(() => {
  //   if (userId && (recommended || popular || trending)) {
  //     fetchData();
  //   }
  // }, [userId, recommended, popular, trending]);

  // Fetch data from the API endpoint
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `https://remsana-backend-testing.azurewebsites.net/api/v1/students-recommendation/${userId}`
      );
        ...
        {courses.map((item, index) => (
          <div className="item px-md-1 course-card" key={item._id}>
            <CourseCard
              ...
              extraclass="mx-2"
              link={`/student/single-course?id=${item._id}`} // Adjust the link to match your routing
            />
          </div>
        ))}
      </Slider>
