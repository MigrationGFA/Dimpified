      ...
      cell: ({ row }) => {
        if (accessorKey === "title") {
          return (
            ...
            >
              <h4 className="mb-1 text-primary-hover">{row.original.itemDetails?.title}</h4>
          ...
          return (
            <Fragment>
              <p className="mb-1 text-primary-hover">{row.original.itemDetails?.category} {row.original.productType}</p>
            </Fragment>
          );
  ...
  return data ? (
    <Fragment>
      <TanstackTable
        ...
        columns={columns}
        filter={true}
        filterPlaceholder="Search Payment History"
        pagination={true}
      />
    </Fragment>
  ) : (
